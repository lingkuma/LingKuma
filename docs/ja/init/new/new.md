...
1. ## フローティングボタン＆パフォーマンス最適化
各ページのハイライト状態は独立しており、ハイライトはデフォルトでオフです。有効化していない間はパフォーマンスを消費しません。
ハイライトをデフォルトでオフにしておくことで、拡張機能のメモリ使用量を抑えられます。手動でハイライトをクリックして有効化した後にのみ、コア機能が読み込まれます。
注：すでに読み込まれているページでは、ハイライトをオフにしてもメモリは解放されません。拡張機能のメモリを解放するには、ハイライトをオフにしてからページを再読み込みする必要があります。
